...
class PCAmodel:
    ...
    def pcabuild(data,dim):
        nd = dim
        pca_l = PCA(n_components=nd, svd_solver= 'full')
        pca_l.fit(data)
        ...
        img_reduced = pca_l.inverse_transform(trans_pca_l)

        return (trans_pca_l, explained_var_ratio, img_reduced)
    ...
    def build_nonlin(data,dim):
        ...
        explained_var_ratio = explained_var / np.sum(explained_var)
        img_reduced = pca_nl.inverse_transform(trans_pca_nl)
        
        return (trans_pca_nl, explained_var_ratio, img_reduced)